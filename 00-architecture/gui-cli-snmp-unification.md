...
- **Controller/GUI-primacy vs. direct-CLI durability.** A third axis, distinct from the two above, surfaced across Ubiquiti/Netgear/Zyxel during vendor research: for some devices, a durable, independent CLI-config surface doesn't exist at all, or isn't the real system of record. Ubiquiti UniFi switches accept CLI-typed config over SSH, but the controller (UniFi Network Controller/UniFi OS) silently overwrites it on the next reboot or reprovision — the CLI session is not authoritative even while it's open. Netgear's "Smart Managed"/GS-series and Zyxel's GS1900 are web-GUI-only by design at the low end of their product lines (partial-to-no config-write CLI), with full CLI only appearing on higher tiers (Netgear M-series). This breaks a silent assumption baked into the session model above: that whatever the CLI session reports *is* device state. For these vendors, SignalScope must:
  1. Detect, per device (not just per vendor — product line/tier matters, e.g. Netgear GS-series vs. M-series), whether the CLI surface is durable before treating CLI-typed changes as committed state.
  2. Where the controller/GUI is the true system of record (Ubiquiti), treat the controller's own API as the primary integration path — analogous to a "vendor API" connectivity mode alongside CLI and SNMP, not a fallback from them — and treat direct-device CLI as diagnostic/secondary rather than config-authoritative.
  3. Surface this distinction to the user before they rely on a GUI action's "done" state: a GUI-issued CLI command that a controller will later revert is a materially different guarantee than one that persists, and silently treating them the same would violate this doc's core visibility bet.
