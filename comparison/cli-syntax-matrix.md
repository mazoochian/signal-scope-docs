...
Scope: the seven vendors with full (Tier 1-3, multi-file) documentation — Cisco IOS/IOS-XE, Arista EOS, Extreme EXOS, Huawei VRP, Juniper Junos, HPE Aruba AOS-CX, MikroTik RouterOS. Appendix-tier vendors (D-Link, Fortinet) are noted in a separate short section at the bottom rather than given full matrix rows, since each spans multiple internal dialects that don't collapse into one row (see their `overview.md` files).
...
## Appendix-tier vendors (not given full matrix rows — see their own `overview.md`)
