...
| Task | Command(s) | Notes |
|---|---|---|
| Disable output paging for the session | `no page` | Referenced in HPE community/Ansible-integration discussions as the AOS-CX equivalent of Cisco's `terminal length 0`; per community reports this setting is **not session-scoped** and persists (survives reboot) once set, unlike most vendors' per-session paging toggle — flagged as community-sourced, not confirmed against an official AOS-CX CLI reference page this session. SignalScope should still echo this command explicitly per the "no hidden setup commands" principle (see [connectivity-methods.md](../../00-architecture/connectivity-methods.md)) and verify actual persistence behavior against a live device before assuming it's a one-time global change rather than per-session. |
